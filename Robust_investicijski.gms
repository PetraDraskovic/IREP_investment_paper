...
*cost comes from deviation in forcasted and real energy, buying in DA market and battery cost; revenue comes from selling in DA market
...
*market position and deviation
c(eq3(t,w,s)).. market_pos(t,w) + deviation(t,w,s) =e= dis(t,w,s) - ch(t,w,s) - el_h(t,w,s) + windy_real(t,w,s);
*c(eq3(t,w,s)).. market_pos(t,w) + deviation(t,w,s) =e= dis(t,w,s) - ch(t,w,s) + windy_real(t,w,s);
...
*c(eq10(t,w,s)).. (1-bat(t,w,s))*M =l= dis(t,w,s);
c(eq11(t,w,s)).. dis(t,w,s) =l= (1-bat(t,w,s))*M  ;
c(eq12(t,w,s)).. dis(t,w,s) =l= p_max ;


*robust
*c(eq24).. bal =e=  sum(w,prob_w(w)*sum(s,prob(s) *(sum(t,cropexDA(t,w)*deviation(t,w,s) - z_dual(t,w,s) + cropexDA(t,w) * coeff*y_dual(t,w,s) +
*           (x_h(t,w,s)*lamdba_h)- ((lamdba_voda * x_h(t,w,s) * eff_voda)/1000)) - omega(w,s)*Gamma )));
